...
export class ApplicantSignUp extends Component {
    constructor(props){
      ...
      submit = (event) => {
        ...
        axios.post('http://localhost:5000/api/applicantsignup',data)
        .then(
          res =>{
            if (!res.data.success){
...
<div className="Vid">
  <img src={img1} alt=""/>
    ...
    <div>
          ...
          <label>userName :</label> <input type="text" id='applicantName' onChange={this.changeInput} placeholder="applicantName..." /><br />
          <label>password :</label> <input type="password" id='password' onChange={this.changeInput} placeholder="password..." /><br />
          <label>email :</label> <input type="email" id = 'email' onChange={this.changeInput} placeholder="email..." /><br />
          <label>mobile :</label> <input type="mobile" id = 'mobile' onChange={this.changeInput} placeholder="mobile..." /><br />
          
          {this.state.signUpError ? this.state.signUpError : ''}
          <br/>
          <input type="button" value='submit' id='btn' onClick={this.submit} />
          <p>If you have an account <Link to='/applicantsignin'>Sign In</Link></p>
      </div>
